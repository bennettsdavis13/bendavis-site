...
export default function EditMode() {
  ...
  useEffect(() => {
    fetch('/api/admin/me').then((r) => r.json()).then((j) => setAuthed(!!j.authed)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    if (!editing) {
      ...
      lists.forEach((list) => {
        list.classList.remove('em-list');
        [...list.children].forEach((c) => { c.draggable = false; c.classList.remove('em-item'); });
      });
      return;
    }
    ...
    lists.forEach((list) => {
      ...
      [...list.children].forEach((item) => { item.draggable = true; item.classList.add('em-item'); });
    });
    ...
    const onDragOver = (e) => {
      ...
      for (const child of items) {
        ...
        const dist = Math.abs(offset) + Math.abs(offX);
  ...
  async function save() {
    ...
    try {
      const cr = await fetch('/api/admin/content');
      const cj = await cr.json();
      if (!cj.ok) { setMsg('Could not load content.'); setBusy(false); return; }
      const content = cj.content;
      document.querySelectorAll('[data-edit]').forEach((el) => {
        try { setIn(content, el.getAttribute('data-edit'), el.innerText.replace(/ /g, ' ').trim()); } catch {}
      });
  ...
  return (
    <>
      {!editing ? (
        <button className="em-fab" onClick={() => setEditing(true)} aria-label="Edit site">✏️ Edit site</button>
      ) : (
